...
- [4.已声明的ID3v2帧](#4已声明的id3v2帧)
  - [4.1.唯一文件标识符(UFID)](#41唯一文件标识符ufid)
  - [4.2.文本信息帧("T000"-"TZZZ"，"TXXX"除外)](#42文本信息帧t000-tzzztxxx除外)
  - [4.2.1.标识帧](#421标识帧)
    - [TIT1](#tit1)
    - [TIT2](#tit2)
    - [TIT3](#tit3)
  ...
  - [4.2.5.其他文本帧](#425其他文本帧)
    ...
    - [TSOP](#tsop)
    - [TSOT](#tsot)
  - [4.2.6.用户定义的文本信息帧(TXXX)](#426用户定义的文本信息帧txxx)
  - [4.3.URL链接帧("W000"-"WZZZ"，"WXXX"除外)](#43url链接帧w000-wzzzwxxx除外)
  - [4.3.1.URL链接帧 - 详情](#431url链接帧---详情)
    ...
    - [WPAY](#wpay)
    - [WPUB](#wpub)
  - [4.3.2.用户定义的URL链接帧(WXXX)](#432用户定义的url链接帧wxxx)
  - [4.4.音乐CD标识符(MCDI)](#44音乐cd标识符mcdi)
  - [4.5.事件定时代码(ETCO)](#45事件定时代码etco)
  - [4.6.MPEG位置查找表(MLLT)](#46mpeg位置查找表mllt)
  - [4.7.同步节奏代码(SYTC)](#47同步节奏代码sytc)
  - [4.8.非同步歌词/文本转录(USLT)](#48非同步歌词文本转录uslt)
  - [4.9.同步歌词/文本(SYLT)](#49同步歌词文本sylt)
  - [4.10.备注(COMM)](#410备注comm)
  - [4.11.相对音量调整(2)(RVA2)](#411相对音量调整2rva2)
  - [4.12.均衡(2)(EQU2)](#412均衡2equ2)
  - [4.13.混响(RVRB)](#413混响rvrb)
  - [4.14.附加图片(APIC)](#414附加图片apic)
  - [4.15.一般封装对象(GEOB)](#415一般封装对象geob)
  - [4.16.播放计数器(PCNT)](#416播放计数器pcnt)
  - [4.17.流行值(POPM)](#417流行值popm)
  - [4.18.推荐缓冲区大小(RBUF)](#418推荐缓冲区大小rbuf)
  - [4.19.音频加密(AENC)](#419音频加密aenc)
  - [4.20.链接信息(LINK)](#420链接信息link)
  - [4.21.位置同步(POSS)](#421位置同步poss)
  - [4.22.使用条款(USER)](#422使用条款user)
  - [4.23.所有权(OWNE)](#423所有权owne)
  - [4.24.商业(COMR)](#424商业comr)
  - [4.25.加密方法注册(ENCR)](#425加密方法注册encr)
  - [4.26.分组标识注册(GRID)](#426分组标识注册grid)
  - [4.27.专用帧(PRIV)](#427专用帧priv)
  - [4.28.签名(SIGN)](#428签名sign)
  - [4.29.查找帧(SEEK)](#429查找帧seek)
  - [4.30.音频搜索点索引(ASPI)](#430音频搜索点索引aspi)
- [5.版权](#5版权)
- [6.参考文献](#6参考文献)
- [7.附录](#7附录)
  - [A.附录A - ID3v1中的流派列表](#a附录a---id3v1中的流派列表)
  ...
  - [C.附录C - 非标准帧](#c附录c---非标准帧)
    ...
    - [C.2.回放增益相关帧](#c2回放增益相关帧)
      - [C.2.1.回放增益简介](#c21回放增益简介)
      - [C.2.2.回放增益的存储方案](#c22回放增益的存储方案)
      - [C.2.3.回放增益TXXX帧定义](#c23回放增益txxx帧定义)
...
帧头后面的文本与非同步歌词/文本转录有一个主要区别。每个音节(或编码器认为方便的文本大小)都是一个空尾字符串，后面跟着一个时间戳，表示它在声音文件中的位置。因此，每次同步的结构如下：
...
$O_i$是在时间偏移量为($i/N*\text{持续时间}$)的点之后起始时间最快的帧的偏移量。
...
这个问题有一个解决方案：在每个音频文件中，可以存储有关以标准响度播放每个曲目或专辑所需的音量变化的信息，并且播放器可以使用此"回放增益(ReplayGain)"信息根据需要自动调高或调低音量。

回放增益规范是一项标准，它定义了适当的参考水平，解释了计算和表示给定曲目或专辑的理想回放增益的方法，并为播放器在播放过程中进行必要的音量调整提供了指导。该标准还规定了当计算出的回放增益超出数字音频限制时防止削波的方法，并描述了回放增益信息在音频文件中的存储方式。

#### C.2.2.回放增益的存储方案

首选方法是使用TXXX帧。另外两种用于存储回放增益元数据的传统方案是RGAD和RVA2。如果在ID3v2标签中找不到TXXX格式的元数据，播放器可以选择查找另外两种。如果新软件希望与旧播放器保持向后兼容，除了写入较新的 (TXXX) 格式外，还可以写入这些较旧的格式。
...
与回放增益元数据相关的四个帧使用以下键/值对

|含义|键|值格式|
|-|-|-|
|音轨回放增益|REPLAYGAIN_TRACK_GAIN|`[-]a.bb dB`|
|音轨峰值振幅|REPLAYGAIN_TRACK_PEAK|`c.dddddd`|
|专辑回放增益|REPLAYGAIN_ALBUM_GAIN|`[-]a.bb dB`|
...
为方便将来扩展，如果存储了三个以上的值，播放器应忽略那些无法识别的值，只理那些可以识别的值。如果要存储除音轨和专辑之外的其他回放增益调整值，则应放在音轨和专辑之后。峰值振幅必须始终占据回放增益标题帧的前4个字节。所有回放增益头都必须包含上述三个值(或至少包含这三个值的字段，如果值本身未知)。这些值与ID3v2的其他数值一样以大端序存储。

回放增益调整必须在`-51.0 dB`和`+51.0 dB`之间。超出此范围的值必须限制在该范围内，但这些值肯定是错误的，可能应重新计算或存储为"未设置"。例如，要使一个无声的24位文件以`83 dB`的音量播放，回放增益调整值将为`+57 dB`。

实际上，`-23 dB`至`+17 dB`之间的调整值可能是极端值，`-18 dB`至`+2 dB`之间的值比较常见。

每个回放增益值都应存储在由两个字节(16位)组成的回放增益调整字段中。下面是两个回放增益调整字段示例：
...
如果文件头中已为回放增益预留空间，但未进行回放增益计算，则所有位(包括名称代码)都可以为0。

对于每个回放增益调整字段，如果名称代码是000(未设置)，则播放器应忽略该单独字段的其余部分。

对于每个"回放增益调整"字段，如果名称代码是一个无法识别的值(即不是`001`(音轨)或`010`(专辑))，则播放器应忽略该个别字段的其余部分。

如果找不到有效的"回放增益调整"字段(即所有名称代码都是`000`或未知)，那么播放器就应该按照文件不包含"回放增益调整"信息的情况继续播放(请参阅播放器要求)。

**发起者代码**

+ `000` 未指定回放增益
+ `001` 艺术家/制作人/母带制作工程师预设的回放增益
+ `010` 用户设置的回放增益
+ `011` 自动确定回放增益，如计算(上文)所述

所有其他的值都保留供将来使用。

对于每个回放增益调整字段，如果名称代码有效，但发起者代码为`000`(回放增益未指定)，则播放器应忽略该回放增益调整字段。

对于每个回放增益调整字段，如果名称代码有效，但发起者代码未知，则播放器仍应使用该回放增益调整字段内的信息。这是因为，即使我们不确定调整是如何确定的，但任何有效的"回放增益调整"都比没有更有用。

如果没有找到有效的"回放增益调整"字段(即所有发起者代码均为`000`)，则播放器应按照文件不包含"回放增益调整"信息的情况继续播放(请参阅播放器要求)。
...
`$00 $00` 应在未计算或设置回放增益的情况下使用。播放器对该值的解释与文件头中没有回放增益字段的文件相同(参见播放器要求)。

`xxxyyy0000000000`(其中`xxx`是任何名称代码，`yyy`是任何发端代码)的值都是有效的，但表示回放增益将保持在`83 dB`(回放增益调整为`0 dB`)。这些值不是默认值，只能在适当的情况下使用(例如，用户、制作者或回放增益计算显示正确的回放增益为`83 dB`)。
...
表演者，一般与[TPE1](#tpe1)同义，有些程序使用此帧存储合作者(Feat.)。
...
### E.2.ID3v2.2和ID3v2.3中都存在但不同的帧

#### E.2.1.[UFID](#41唯一文件标识符ufid)

如果在"帧大小"后面发现`$00`，则整个帧将被忽略，最好删除。

#### E.2.2.[TFLT](#tflt)

VQF和PCM为ID3v2.3新增预定义类型。

#### E.2.3.[ETCO](#45事件定时代码etco)

`$0E-DF`保留供将来使用。

#### E.2.4.[USLT](#48非同步歌词文本转录uslt)

内容描述符的最大长度为64字节。

#### E.2.5.[SYLT](#49同步歌词文本sylt)

"`$06`小节/弹出式信息"为ID3v2.3新增。

#### E.2.6.[RVAD](#b38相对音量调整rvad)

ID3v2.2中仅定义了两个声道，"增量/减量"字段为`%000000xx`。

#### E.2.7.[EQUA](#b39均衡equa)

增量/减量位为`1`表示增量，`0`表示减量。均衡波段应根据频率递增排序。不必声明所有频率。应忽略值为0的调整。一个频率只能在帧中描述一次。

#### E.2.8.[APIC](#414附加图片apic)

ID3v2.2中不存在"MIME类型"字段，在其位置的为"图像类型"字段，长度为固定的三个字节。

图片格式最好为`"PNG"`或`"JPG"`。若该帧指示链接，则"图像类型"字段为`"-->"`。

### E.3.ID3v2.3弃用的ID3v2.2中的帧

#### E.3.1.被加密元数据(CRM)

该帧包含一个或多个加密帧。这样就可以保护受版权保护的信息，如图片和文字，人们可能希望为这些信息支付额外费用。由于此类加密方案的标准化工作不在本文档范围内，因此所有"CRM"帧都以包含电子邮件地址的URL[URL]字符串开头，或以可找到电子邮件地址的位置链接开头，该地址属于负责此特定加密元数据帧的组织。

有关该帧的问题应发送至指定的电子邮件地址。如果在"帧大小"后直接出现`$00`，则整个帧应被忽略，最好删除。在"所有者标识符"之后是简短的内容描述和加密原因说明。在"内容/说明"描述之后，是实际的加密块。

ID3v2解码器遇到"CRM"帧时，应将数据块与相应的"所有者标识符"一起发送到"插件"，并期待收到一个或多个ID3v2帧相继出现的数据块，或者收到一个错误。一个标签中可能有多个"CRM"帧，但只有一个具有相同的"所有者标识符"。
...
表中不含[CRM](#e31被加密元数据crm)帧，因为其在ID3v2.3中被删除。

|ID3v2.2名称|ID3v2.3名称|
|-|-|
|BUF|[RBUF](#418推荐缓冲区大小rbuf)|
|CNT|[PCNT](#416播放计数器pcnt)|
|COM|[COMM](#410备注comm)|
|CRA|[AENC](#419音频加密aenc)|
|ETC|[ETCO](#45事件定时代码etco)|
|EQU|[EQUA](#b39均衡equa)|
|GEO|[GEOB](#415一般封装对象geob)|
|IPL|[IPLS](#b37参与人员名单ipls)|
|LNK|[LINK](#420链接信息link)|
|MCI|[MCDI](#44音乐cd标识符mcdi)|
|MLL|[MLLT](#46mpeg位置查找表mllt)|
|PIC|[APIC](#414附加图片apic)|
|POP|[POPM](#417流行值popm)|
|REV|[RVRB](#413混响rvrb)|
|RVA|[RVAD](#b38相对音量调整rvad)|
|SLT|[SYLT](#49同步歌词文本sylt)|
|STC|[SYTC](#47同步节奏代码sytc)|
|TAL|[TALB](#talb)|
|TBP|[TBPM](#tbpm)|
|TCM|[TCOM](#tcom)|
|TCO|[TCON](#tcon)|
...
|TT3|[TIT3](#tit3)|
|TXT|[TEXT](#text)|
|TXX|[TXXX](#426用户定义的文本信息帧txxx)|
|TYE|[TYER](#b36tyer)|
|UFI|[UFID](#41唯一文件标识符ufid)|
|ULT|[USLT](#48非同步歌词文本转录uslt)|
|WAF|[WOAF](#woaf)|
|WAR|[WOAR](#woar)|
|WAS|[WOAS](#woas)|
|WCM|[WCOM](#wcom)|
...
瑞兹路(Rydsvägen) 246 C. 30 SE-584 34 林雪平(Linköping) 瑞典
