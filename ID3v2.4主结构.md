...
如果设置了该标志，扩展头中将包含CRC-32[ISO-3309]数据。CRC 是对页眉和标签尾之间的所有数据计算的，如页眉的标记长度字段所示，减去扩展页眉。请注意，这包括填充（如果有的话），但不包括标签尾。CRC-32以35位同步安全整数形式存储，上四位始终为零。
...
在最后一帧后（ID3 标签的末尾）加入填充是可选项，可使所有帧的尺寸加在一起小于标签头中给出的尺寸。这种填充的一个可能目的是在标签中添加一些额外的帧或扩大现有的帧，而不必重写整个文件。填充字节的值必须是`$00`。标签的帧与帧之间或标签头与帧之间不得有任何填充。此外，当标签标签尾添加到标签中时，也不得有任何填充。
...
其中"扩展头大小"（目前为 6 或 10 字节）不包括自身。"填充大小"指的是不包括帧和标签头的总标签大小，即填充大小。扩展头与标头本身是分开的，因此会出现不同步的情况。
