...
通过该帧可以对以"组识别注册"分组的一组帧进行签名。虽然签名可以放在登记框内，但也可能需要将签名存储在其他地方，如水印中。一个标签中可以有多个"SIGN"帧，但不能有两个完全相同的签名框。
...
该帧表示文件/数据流中其他标记的位置。到下一个标记的最小偏移量"是从这个标记的末尾到下一个标记的起始点计算出来的。一个标记中只能有一个"SEEK"帧。
...
**以下内容部分引自[维基百科](https://zh.wikipedia.org/)，这些内容使用[CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)授权。**
